...
- `textures/ui/tavern/`：团队提供的 960×540 酒馆大厅候选和单独的酒保 NPC 候选；旧 Cozy Tavern 背景已移入 `art_sources/legacy/tavern/`。
- `textures/ui/restaurant/restaurant_interior.png`、`textures/ui/home/home_interior.png`、`textures/ui/library/library_interior.png`、`textures/ui/tavern/tavern_lobby.png`、`textures/ui/convenience_store/convenience_store_interior.png`：团队提供的室内场景合成稿经裁切/最近邻处理后的 960×540 运行时候选；源图、哈希和授权边界见 `art_sources/manual_composites/README.md`。
