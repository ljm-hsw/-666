...
从地图点击餐馆、便利店、图书馆或家后，会先进入不消耗行动的场景大厅。餐馆和便利店可点击固定 NPC/主要活动按钮或按 `Enter`/`Space` 打开短对话；图书馆可点击管理员或按 `Space` 打开短对话。三者关闭或跳过后才进入原有玩法，家仍显示预留互动。大厅可以直接返回地图且不消耗阶段，酒馆继续使用自己的大厅交互。

确认新游戏后会在第一日地图上显示镇长与主角的四句开场；`Enter`、`Space`、鼠标点击继续，`Esc` 跳过。第一日未执行白天行动的存档恢复允许无奖励重播。夜晚进入家并确认休息后会先显示两句主角独白，关闭或跳过后才提交一次休息结果并进入每日总结。
