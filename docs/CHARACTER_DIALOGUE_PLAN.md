...
| 餐馆开始工作前 | 餐馆老板、主角 | 围裙、订单和服务节奏，2-3 句 | 倒计时在对话关闭前不启动 |
| 便利店锁定方案前 | 便利店店主、主角 | 天气、库存和账本，2-3 句 | 对话关闭后才允许经营输入 |
| 图书馆室内靠近管理员 | 管理员、主角 | 分类、借书卡和旧地图，2-3 句 | 对话结束后进入现有答题流程 |
| 酒馆室内靠近酒保 | 酒保、主角 | 桌游、赌注和夜晚规矩，2-3 句 | 迁移现有酒保硬编码弹窗 |
| 选择回家休息 | 主角独白 | 热水、日记和明天，1-2 句 | 不改变既有回家收益与总结 |
...
- 图书馆：保留题库和规则引擎，把 `npc_talk` 文案与弹窗迁移到共享对话系统；随后接入固定室内移动，靠近管理员后进入既有对话与答题。图书馆专用 `NpcManager` 不升级为全项目领域 NPC 系统。
...
5. Issue 27：接入图书馆固定室内移动、书架/家具碰撞、管理员轨迹和对话到答题的完整路径。
6. Issue 28：复用导航边界接入酒馆室内移动、吧台/桌椅碰撞、常客轨迹和既有挑战入口。
7. Issue 16：结合试玩批准 NPC 文案长度、语气、内容密度和室内移动手感；不新增复杂关系系统。
8. Issue 17：统一替换最终人物、对话框装饰和地点视觉，并同步验收家具/书架等视觉物件与碰撞体，完成来源、hash、许可和人工视觉验收。
9. Issue 18：在 Windows 发布包和演示路径中验证全部对话、移动、碰撞、fallback、字体与资源清单。
