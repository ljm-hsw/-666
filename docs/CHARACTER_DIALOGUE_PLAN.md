...
2026-07-14 架构收敛：项目取消主角室内移动、NPC 巡逻、寻路和运行时碰撞。所有 NPC 固定在柜台或作者指定热点循环待机动画，玩家通过点击人物或场景热点进入共享对话和既有玩法。2026-07-15 餐馆、便利店、图书馆和酒馆统一接入团队提供的四帧固定 NPC 候选；程序 fallback、最终授权和 sprite 保留决定仍由 Issue 17 人工批准。
...
当前运行时使用 `<role>_idle_sheet.png`，由 `NpcSpriteSpec` 统一登记路径、帧尺寸、帧数和持续时间。餐馆厨师、便利店售货员、图书管理员和酒保均为 `4×64×96`；Issue 17 再决定是否保留、重绘或替换，地点页面不得自行写死另一套帧参数。
...
酒馆纹理继续由 `TavernVisualAssets` 延迟加载，其他三个地点纹理由 `SceneVisualAssets` 集中加载；四者共享 `NpcSpriteSpec` 和帧绘制函数。资源缺失时保留共享程序 fallback，资源路径与帧参数不进入规则层。
