...
9. 固定 NPC 场景运行期只接收帧时间、点击与对话输入并提供只读 presentation；它不拥有移动、寻路、碰撞或地点规则。
10. NPC 固定热点和待机动画时间属于应用展示状态，不得写入 `PlayerState`、地点规则、行动结果或 v1 存档。
...
所有地点都使用固定镜头、固定热点或面板交互。首个可交付版本不提供主角室内移动，也不提供 NPC 巡逻、寻路或动态碰撞。

NPC 固定在柜台或作者指定位置，展示层根据运行期提供的待机动画时间循环播放 sprite sheet；缺少正式人物资源时使用程序占位动画。鼠标点击人物热点触发共享短对话，固定按钮或桌面热点进入既有玩法。热点不修改玩家状态，只有既有地点规则产生并提交行动结果。

对话 active 时先由对话 Runtime 消费 Enter、Space、Esc 和鼠标输入，不能同时触发返回、玩法按钮或第二次 NPC 互动。暂停、失焦或最小化时，NPC 待机动画和地点计时遵循全局冻结规则。

餐馆、家、图书馆、酒馆和便利店的静态碰撞元数据只保留给 F3 美术对齐诊断，不进入任何玩法判断，也不作为 NPC 热点或发布门禁。门洞碰撞框清理继续保留为诊断数据修正，但取消移动后不会影响实际交互。

当前 CMake 将 `dialogue_runtime` 与 `library_room_runtime` 编入不链接 raylib 的 `pixel_town_dialogue_logic`。图书馆展示层只把固定管理员热点点击或 Space 转为显式意图，读取待机动画时间和对话 presentation；共享管理员对话关闭后才由 `location_runtime` 进入既有读者咨询/书籍整理模式选择。酒馆沿用既有固定酒保六帧待机动画与点击热点。
...
- 鼠标为主要输入，常用操作提供键盘快捷键。
- 不提供主角移动控制；NPC 固定在作者指定热点循环待机动画，鼠标点击 NPC 或地点热点是主要互动方式，Space 可作为当前固定 NPC 的等价快捷键。
...
- 通用场景大厅属于应用内临时展示状态，不写入 `GameSession` 或 v1 存档；退出或重启时安全回到对应阶段地图。尚未接入正式脚本的占位 NPC 热点只反馈“接口已预留”，不得提前修改属性或地点规则。
...
- 图书馆和酒馆各有一条“固定场景 -> 点击 NPC/热点 -> 对话或玩法 -> 返回场景”的集成路径。
