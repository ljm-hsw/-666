...
- 图书馆启动时从正式 `library_data.txt` 汇总题目、选项和剧情标点，再与静态 UI 字形合并、去重后加载字体；数据文件中的 `？`、引号等字符不得退化为替代字形。
...
`EndingRules` 是 raylib-free 的最终结算 Module。默认十日版读取第十日夜晚结算后的玩家状态、便利店库存和酒馆战绩，先按集中商品成本与清算比例计算现金，再使用清算后的金钱判定唯一主结局。`GameSession` 只有在配置的最终日 `DaySettlement` 阶段接受该结果，随后清空库存并进入 `Ending`；已经进入结局的会话不能再次清算。正式默认值固定为 10，五日展示构建通过 `PIXEL_TOWN_DAY_LIMIT=5` 压缩最终日，但不改变行动结果、数值规则或结局优先级。
...
- 旧集市地图是达到知识门槛后的第二次图书馆访问事件：玩家收下后本次运行立即确认；恢复存档时以已完成图书馆访问次数判定其已经展示，第三次及以后访问不再重复弹出。地图图片是无文字候选美术，标题和说明仍使用统一字体绘制。
...
- `game_config`：编译期产品变体边界；只允许正式十日版和五日展示版。五日展示版必须使用独立构建目录和输出名，不能把展示日数写成地点或 UI 私有常量。
...
家中确认休息会从同一地点事件目录选择当日晚访客脚本。首次访客负责解释休息语义，第 2-10 天复用已批准 NPC 形成固定来访日程；访客对话关闭或跳过后才由 `StoryLifecycleRuntime` 委托既有 `home_rest_result()`，因此不新增阶段、存档字段或第二次夜晚结算。
...
- 发布运行优先使用可执行文件旁的 `assets/` 作为运行根目录；开发构建旁没有资源时回退到启动工作目录。存档、设置、日志和诊断截图因此都留在便携发布目录，不依赖 Windows 快捷方式的“起始位置”。
...
- MSVC 对全部源码显式启用 UTF-8，避免中文字符串随系统代码页产生乱码或编译差异。
- raylib 6.0 和 doctest 2.5.2 使用仓库内锁定版本，首次构建不访问网络。
- CMake 安装目标生成“可执行文件 + 运行时 assets + CREDITS + 许可证”的便携目录；十日版与五日展示版使用不同安装前缀。
