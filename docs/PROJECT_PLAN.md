...
2026-07-09 集成说明：`codex/integrate-team-branches` 已合入餐馆和图书馆模块的规则、基础 UI 和测试。便利店仍未合入，因此 P2 阶段仍未整体完成。餐馆/图书馆后续需要统一视觉、叙事摘要入口和应用层拆分，详见 [`TEAM_BRANCH_INTEGRATION.md`](TEAM_BRANCH_INTEGRATION.md)。
...
- P2/P3 接入真实地点、剧情文本、截图或持久化增强时，优先拆出窄模块（如 `AppPersistence`、`CaptureService`、`MapScene`/`TitleScene`、`StoryText`），避免继续扩大 `main.cpp` 和 `game_flow.cpp` 单体职责。
