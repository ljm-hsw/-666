...
人物小人、轻量对话和有限室内移动采用可替换占位视觉先完成基础功能，再由最终视觉 issue 统一替换。共享边界、32×32 占位帧、方向键输入、静态碰撞、NPC 路点、触发矩阵、输入拦截和存档约束见 [`CHARACTER_DIALOGUE_PLAN.md`](CHARACTER_DIALOGUE_PLAN.md)。Issue 23-29 是可自动实施的垂直切片；人物造型、移动手感、最终文案密度和视觉/碰撞一致性仍分别由 Issue 16/17 人工批准。

2026-07-11 临时执行调整：Issue 23 保持完成；Issue 24-28 最初改为 `needs-info`，先按 [`SCENE_ASSET_PRODUCTION_PLAN.md`](SCENE_ASSET_PRODUCTION_PLAN.md) 完成视觉输入。2026-07-13 团队提供的餐馆、家、图书馆、酒馆和便利店 960×540 合成稿已接入；图书馆/酒馆静态碰撞、出生点、出口和诊断图已落地，Issue 27/28 因此恢复为 `ready-for-agent`，但 28 继续受 27 依赖阻塞。Issue 24-26 仍保持 `needs-info`，其既有依赖和验收标准不因此失效。
...
2026-07-14 图书馆玩法调整：Issue 29 已以新增模式接入书籍整理，原 Issue 11 读者咨询继续保留；进入图书馆行动后先选择模式，两者共享数据加载、场景视口和核心统一结算。Issue 27 后续把管理员邻近互动接到该模式选择入口。
...
| [27 接入图书馆室内移动、静态碰撞与管理员轨迹](../.scratch/pixel-town-ten-day-plan/issues/27-p4-library-room-navigation.md) | AFK | 11、23 | 方向键移动、书架/家具碰撞、管理员轨迹和对话到答题路径 |
| [28 接入酒馆室内移动、静态碰撞与常客轨迹](../.scratch/pixel-town-ten-day-plan/issues/28-p4-tavern-room-navigation.md) | AFK | 22、23、27 | 复用导航边界、吧台/桌椅碰撞、酒保/常客与挑战入口 |
...
| [16 完成内容扩充与数值平衡](../.scratch/pixel-town-ten-day-plan/issues/16-p4-content-and-balance.md) | HITL | 09-15、22、24-29 | 固定种子模拟、人物文案/移动试玩记录和批准参数 |
| [17 完成像素美术、音频、教程与署名](../.scratch/pixel-town-ten-day-plan/issues/17-p4-presentation-and-credits.md) | HITL | 03、09-11、13-15、22-29 | 最终人物/场景资源、视觉/碰撞一致性、许可、教程和人工验收 |
