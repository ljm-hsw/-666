...
2026-07-09 集成说明：`codex/integrate-team-branches` 已合入餐馆、便利店和图书馆模块的规则、基础 UI 和测试。地点结果映射和地点运行期流程已先拆到 `location_result_adapter` / `location_runtime`，详见 [`TEAM_BRANCH_INTEGRATION.md`](TEAM_BRANCH_INTEGRATION.md)。餐馆已拆出 UI/反馈和视觉布局 follow-up；便利店当前使用 P2 占位 UI 完成三档价格、进货数量、销售模拟和跨日库存存档，最终商品级 UI 与视觉表现留到后续 polish。
...
2026-07-09 集成说明：`codex/integrate-team-branches` 已合入酒馆规则、赌注、战绩统计、夜晚入口和存档兼容处理。五子棋/骗子骰子的真实玩法仍由 Issue 13/14 承接，当前酒馆结果仍是可控模拟外壳。
