...
`--capture-prototype` 与 `--capture-game-flow` 导出的是 960×540 逻辑画布按最近邻放大后的 1920×1080 图片，不依赖当前机器显示器是否能完整容纳默认窗口。
