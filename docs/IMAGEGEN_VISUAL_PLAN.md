...
2026-07-11 起，酒馆、图书馆和便利店不再优先追求“一次生成完整可用背景”，改用“风格板 -> 空房壳 -> 家具/道具分件 -> 人工拼接 -> 碰撞/热区审查 -> 程序替换”的生产路线。具体批次、prompt、目录、拼接和恢复 Issue 门槛见 [`SCENE_ASSET_PRODUCTION_PLAN.md`](SCENE_ASSET_PRODUCTION_PLAN.md)。本文继续维护项目级风格和画布约束，分件生产细节以后者为准。
...
| P2 | 图书馆室内合成稿 | 960×540 | 图书馆玩法与后续移动 | imagegen 空房壳与分件人工拼接；书架、借阅台、分类区；同步碰撞审查图 |
| P3 | 酒馆室内合成稿 | 960×540 | 酒馆选择/小游戏入口与后续移动 | imagegen 空房壳与分件人工拼接；木质暖光和桌游桌；同步碰撞审查图 |
...
当前项目已临时暂停 Issue 24-28，先完成三地点分件素材和人工合成草稿。这是执行顺序调整，不改变已经通过的 P1-P3 规则验收，也不允许候选美术反向修改地点规则。
