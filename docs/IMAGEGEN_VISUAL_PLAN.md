...
2026-07-11 起，酒馆、图书馆和便利店不再优先追求“一次生成完整可用背景”，改用“风格板 -> 空房壳 -> 家具/道具分件 -> 人工拼接 -> 固定 NPC/玩法热点审查 -> 程序替换”的生产路线。具体批次、prompt、目录、拼接和恢复 Issue 门槛见 [`SCENE_ASSET_PRODUCTION_PLAN.md`](SCENE_ASSET_PRODUCTION_PLAN.md)。本文继续维护项目级风格和画布约束，分件生产细节以后者为准。
...
| P2 | 便利店室内合成稿 | 960×540 | 便利店玩法 | imagegen 空房壳与分件人工拼接；货架、收银台、商品区；不含文字 |
| P2 | 图书馆室内合成稿 | 960×540 | 图书馆固定管理员与两种工作模式 | imagegen 空房壳与分件人工拼接；书架、借阅台、分类区；同步热点审查图 |
| P3 | 酒馆室内合成稿 | 960×540 | 酒保对话与小游戏固定入口 | imagegen 空房壳与分件人工拼接；木质暖光和桌游桌；同步热点审查图 |
...
Issue 24 已用程序人物 fallback 完成餐馆老板固定热点和主线初识对话；最终人物图仍等待 Issue 17 人工批准。Issue 25/26 继续等待对应固定 NPC 文案和视觉输入，Issue 27/28 已按固定管理员、固定酒保与点击热点方案完成。这不改变已经通过的 P1-P3 规则验收，也不允许候选美术反向修改地点规则。
