...
| 场景 | 运行时路径 | 当前接入 | 热点与可选诊断 |
| --- | --- | --- | --- |
| 餐馆 | `assets/textures/ui/restaurant/restaurant_interior.png` | 先显示餐馆大厅，主角可绕开家具靠近固定老板，再进入原有准备、订单和结算 UI | 老板邻近区、出生点、出口和家具碰撞由 Issue 33 固化；F3 可审查 |
| 家 | `assets/textures/ui/home/home_interior.png` | 主角可从门口移动到床边准备休息；返回不消耗阶段 | 床铺邻近区、出生点、出口和家具碰撞由 Issue 33 固化 |
| 图书馆 | `assets/textures/ui/library/library_interior.png` | 主角可绕过书车靠近固定管理员，再选择读者咨询或书籍整理 | 管理员邻近区、出生点、出口和家具碰撞由 Issue 33 固化 |
| 酒馆 | `assets/textures/ui/tavern/tavern_lobby.png` | 固定酒保与双桌热点保留，主角可移动到附近进入对话或玩法 | 酒保/桌面邻近区、出生点、出口和家具碰撞由 Issue 33 固化 |
| 便利店 | `assets/textures/ui/convenience_store/convenience_store_interior.png` | 主角可沿通道靠近固定店主，再进入进货、定价、预算和结算 UI | 店主邻近区、出生点、出口和家具碰撞由 Issue 33 固化 |

固定 NPC、玩法热点、主角出生点、可行走边界和静态碰撞以各场景运行时代码为事实源，使用 960×540 场景源坐标并统一映射到室内视口。`src/core/scene_collision.cpp` 的矩形实际限制主角脚底盒，F3 用于审查与背景对齐；相关无窗口测试验证布局、门洞、出生点、阻挡和滑动契约。

Issue 27/28 已按固定 NPC 方案历史收口；Issue 33 只在其上新增主角有限移动、静态碰撞和邻近互动。图书馆管理员、酒馆酒保及其他 NPC 仍固定在作者指定位置，不恢复 NPC 路点、巡逻或寻路。图片组成素材的授权信息仍待团队补齐，不能据此视为可商用最终美术。
...
- Issue 24-28 已按固定 NPC 或生命周期 overlay 架构完成，各切片互不依赖；最终人物造型仍由 Issue 17 人工批准。
- 当前执行入口改为本计划的视觉生产和人工拼接流程。
- 镇长/主角、餐馆和便利店主线对话切片均已恢复并完成；后续视觉生产只替换批准素材，不反向改动运行期和地点规则。
...
6. 背景源图不得烘焙状态栏；运行时会把完整 960×540 场景等比例放入顶部 60 像素
   ...
   与场景坐标、可见 NPC 和点击热点共同登记。
7. 不在背景中烘焙人物、中文、按钮、对话框、玩法提示和可变数值。
8. 导出一张未叠加程序 UI 的 960×540 runtime candidate，并另外导出一张带 UI、NPC 和热点辅助线的 review PNG；历史碰撞线只在需要美术诊断时可选显示。
...
- 吧台、桌椅、木桶、壁炉和墙体应保持轮廓清楚，便于美术审查，但不要求运行时碰撞。
- 不把五子棋棋盘、骰面或赌注数值烘焙成玩法状态；运行时棋局仍由程序绘制。

### 图书馆

- 管理员岗位、答题入口、返回入口和阅读区必须在画面中可识别，点击热点不得互相遮挡。
- 高书架、矮书架、借阅台、阅读桌和墙体属于背景构图；后续增加家具时只需复查热点可见性和可点击性。
...
- 便利店经营玩法本身仍是静态面板；开始经营前的固定大厅允许有限主角移动，货架、收银台和冰柜必须与静态碰撞对齐。
...
每个含 NPC 的室内场景在人工合成后必须提供一张热点审查图和一份坐标清单。2026-07-14 餐馆固定老板、便利店固定店主、图书馆固定管理员、酒馆固定酒保和桌游热点已经接入；镇长/主角生命周期对话使用地图或家中背景 overlay，不新增室内热点：

- `npc_visual_bounds`：固定 NPC 的实际绘制范围。
- `interaction_zones`：管理员、酒保、桌游桌等可点击范围。
- `proximity_zones`：主角在何处可按 E/Space 触发同一互动。
- `walkable_bounds` / `player_spawn` / `exit_trigger`：可行走边界、门口出生点和退出区。
- `static_colliders`：墙体、柜台、书架、桌椅等实际阻挡矩形。
- `idle_animation`：帧数、播放速度、循环方式和缺失资源 fallback。
- `labels`：悬停标签或提示文字的锚点，不能遮挡相邻热点。
- `scene_actions`：对话结束后进入小游戏、模式选择或返回场景的明确结果。

坐标清单以 960×540 场景源坐标为准，由统一视口完成窗口换算。任何 NPC 或家具视觉位置发生变化时，点击/邻近热点、标签和静态碰撞必须在同一评审批次更新；F3 叠加与实际 WASD 试玩缺一不可。
...
- 固定 NPC 在画面中可辨认，循环待机不会越过柜台或遮挡关键 UI。
- NPC、标签和点击热点对齐，点击不会误触相邻人物或玩法入口。
- 主角能从出生点走到所有必需目标和门口，不能穿过家具，门洞不存在错误阻挡。
...
4. 图书馆、酒馆的固定 NPC 和玩法热点坐标完成；可见人物、标签和热点一一对应。
...
- 不把固定室内场景扩展为滚动地图、开放世界或通用寻路场景；只允许 Issue 33 定义的有限主角移动与静态碰撞。
- 不让 imagegen 输出直接决定热点；坐标由人工合成稿与程序审查共同确定。
