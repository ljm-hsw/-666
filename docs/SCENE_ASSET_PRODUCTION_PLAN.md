...
优先地点固定为酒馆、图书馆和便利店。餐馆继续遵循 [`RESTAURANT_VISUAL_SPEC.md`](RESTAURANT_VISUAL_SPEC.md)，不加入 imagegen 首批生成；2026-07-13 团队另行提供的餐馆、家和图书馆合成稿作为补充接入批次，不改变 imagegen 分件生产路线。

本轮只准备候选素材和人工合成底稿，不改变地点规则、UI 热区、对话逻辑、行动结果或 v1 存档。现有代码绘制和旧背景继续作为 fallback，直到新合成稿通过人工验收。

## 已接入的团队合成稿（2026-07-13）

团队提供的餐馆、家和图书馆 PNG 已归档到 `assets/art_sources/manual_composites/`，并转换为 960×540 运行时图：

| 场景 | 运行时路径 | 当前接入 | 碰撞切分 |
| --- | --- | --- | --- |
| 餐馆 | `assets/textures/ui/restaurant/restaurant_interior.png` | 原有餐馆准备、订单和结算 UI 的背景 | 柜台、果蔬台、木桶、冷柜、两组餐桌、伞架、墙体和门口通道 |
| 家 | `assets/textures/ui/home/home_interior.png` | 地图点击“家”后先进入确认休息界面；返回不消耗阶段 | 床、衣柜、壁炉、扶手椅、圆桌、书架、书桌、右侧桌椅、沙发、墙体和门口通道 |
| 图书馆 | `assets/textures/ui/library/library_interior.png` | 原有引导、对话、答题、反馈、总结及房间视图的背景 | 借阅台、书车、三组书架、地图桌、两组阅读桌、植物、墙体和门口通道 |

碰撞数据的唯一代码事实源为 `src/core/scene_collision.cpp`，使用 960×540 画布坐标；无窗口测试覆盖布局合法性、出生点、门口通道、越界和轴向滑动。运行时按 F3 可显示：绿色可行走边界、红色静态碰撞体、蓝色出口触发区和黄色 24×24 出生占位。

这次接入不包含主角移动、NPC 路点或互动区逻辑。它只冻结可供后续 Issue 27 使用的背景、静态碰撞和出生/出口基础契约，也不改变 v1 存档格式。图片组成素材的授权信息仍待团队补齐，不能据此视为可商用最终美术。

## 临时执行顺序

- Issue 23 已完成，不回退其共享对话能力。
- Issue 24-26 与 28 暂时保持 `needs-info`，表示等待本计划中的人工视觉输入和用户恢复确认；Issue 27 已因团队提供图书馆合成稿和静态碰撞清单而恢复为 `ready-for-agent`。
...
6. 顶部 `0-84` 为全局状态栏安全带，不放角色、互动目标或关键提示物件。
...
图书馆和酒馆在人工合成后必须各自提供一张碰撞/互动审查图和一份坐标清单。2026-07-13 图书馆已先提供静态碰撞、出生点和出口；互动区与 NPC 路点仍留给 Issue 27：
...
坐标清单以程序当前 640×360 设计网格为主，并附 960×540 画布换算结果。任何阻挡物视觉位置发生变化时，碰撞清单必须和合成稿在同一评审批次更新。便利店不提供移动碰撞清单，只记录程序面板安全区。
...
除已按用户确认单独恢复的 Issue 27 外，同时满足以下条件后，才批量恢复仍暂停的视觉相关 Issue：
