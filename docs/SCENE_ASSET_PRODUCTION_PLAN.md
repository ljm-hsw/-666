...
| 便利店 | `assets/textures/ui/convenience_store/convenience_store_interior.png` | 先显示便利店大厅与店主预留热点，再进入原有进货、定价、预算和结算 UI | 店主热点待 Issue 25 固化；家具范围仅作可选 F3 审查 |
...
- Issue 24 已按固定 NPC 点击架构完成；Issue 25/26 暂时保持 `needs-info`。Issue 27 和 28 已标记为 `done`，各地点切片互不依赖。
- 当前执行入口改为本计划的视觉生产和人工拼接流程。
- 只有满足“恢复 Issue 的门槛”并得到用户明确确认后，才继续便利店和镇长对话的固定 NPC 切片；餐馆切片已经恢复并完成。
...
每个含 NPC 的室内场景在人工合成后必须提供一张热点审查图和一份坐标清单。2026-07-14 餐馆固定老板、图书馆固定管理员、酒馆固定酒保和桌游热点已经接入；便利店店主和镇长继续由 Issue 25/26 承接：
