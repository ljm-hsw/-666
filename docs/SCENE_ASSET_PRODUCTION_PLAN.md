...
| --- | --- | --- | --- |
| 餐馆 | `assets/textures/ui/restaurant/restaurant_interior.png` | 先显示餐馆大厅与老板固定热点、完成短对话，再进入原有准备、订单和结算 UI | 老板热点和程序 fallback 已由 Issue 24 固化；家具范围仅作可选 F3 审查 |
| 家 | `assets/textures/ui/home/home_interior.png` | 先显示家中大厅与互动预留热点，再进入休息确认；两次返回都不消耗阶段 | 休息热点与返回入口；家具范围仅作可选 F3 审查 |
| 图书馆 | `assets/textures/ui/library/library_interior.png` | 先显示图书馆大厅与固定管理员热点，再选择读者咨询或书籍整理；两种模式共享背景视口 | 管理员热点已固化；家具范围仅作可选 F3 审查 |
| 酒馆 | `assets/textures/ui/tavern/tavern_lobby.png` | 替换旧酒馆背景；固定酒保、五子棋桌和骗子骰子桌热点已重新对齐 | 酒保与桌游热点已固化；家具范围仅作可选 F3 审查 |
| 便利店 | `assets/textures/ui/convenience_store/convenience_store_interior.png` | 先显示便利店大厅与店主固定热点、完成短对话，再进入原有进货、定价、预算和结算 UI | 店主热点和程序 fallback 已由 Issue 25 固化；家具范围仅作可选 F3 审查 |

固定 NPC 和玩法热点以各场景运行时代码为事实源，使用 960×540 场景源坐标并统一映射到
室内视口。`src/core/scene_collision.cpp` 中的历史碰撞清单不参与人物移动或玩法判断，只保留
为可选 F3 美术审查资料；相关无窗口测试只验证诊断矩形合法、门口不被误标等静态契约。

这次接入不包含主角移动、NPC 路点或运行时碰撞。Issue 27/28 已按固定 NPC 方案收口：图书馆管理员和酒馆酒保固定在作者指定位置循环待机，玩家通过点击热点进入对话或既有玩法。历史碰撞数据只用于可选 F3 视觉审查，不代表任何地点开放人物移动。图片组成素材的授权信息仍待团队补齐，不能据此视为可商用最终美术。

## 临时执行顺序

- Issue 23 已完成，不回退其共享对话能力。
- Issue 24/25 已按固定 NPC 点击架构完成；Issue 26 暂时保持 `needs-info`。Issue 27 和 28 已标记为 `done`，各地点切片互不依赖。
- 当前执行入口改为本计划的视觉生产和人工拼接流程。
- 只有满足“恢复 Issue 的门槛”并得到用户明确确认后，才继续镇长/主角生命周期对话切片；餐馆和便利店切片已经恢复并完成。
...
每个含 NPC 的室内场景在人工合成后必须提供一张热点审查图和一份坐标清单。2026-07-14 餐馆固定老板、便利店固定店主、图书馆固定管理员、酒馆固定酒保和桌游热点已经接入；镇长继续由 Issue 26 承接：
...
坐标清单以 960×540 场景源坐标为准，由统一视口完成窗口换算。任何 NPC 视觉位置发生变化时，点击热点和标签必须和合成稿在同一评审批次更新。历史静态碰撞清单可继续用于 F3 美术诊断，但不参与 MVP 玩法。
...
- 不把任何地点扩展为可移动室内地图、滚动地图、开放世界或通用寻路场景。
