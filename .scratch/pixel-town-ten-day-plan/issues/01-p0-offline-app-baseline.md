...
- [ ] macOS 使用同一 CMake 入口可完成配置与构建。
- [ ] 首次配置和构建在依赖已随仓库存在的情况下不访问网络，也不要求全局安装 raylib 或 doctest。
- [ ] 应用启动单一窗口，以 640×360 逻辑画布清晰显示一个可识别的基线场景，并默认呈现在 1280×720 窗口中。
- [ ] CTest 能发现并运行至少一个不创建图形窗口的 doctest 测试。
- [ ] Windows 与 macOS GitHub Actions 执行配置、构建和 CTest。
- [ ] 第三方版本与许可证可从仓库直接确认。
- [ ] 构建说明记录标准命令、支持环境和离线前提。
