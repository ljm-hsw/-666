...
- [ ] 玩家从酒馆选择五子棋后完成一局，结束后返回统一酒馆结算。
- [ ] 已占用、越界或非当前回合落子被拒绝且不改变棋盘。
- [ ] 横、竖和两种斜线五连均正确判胜，棋盘满且无人获胜时判平局。
- [ ] 不实现禁手，玩家和电脑遵循同一落子合法性规则。
- [ ] 电脑优先立即获胜，其次立即防守，否则使用稳定位置评分。
- [ ] 相同棋盘状态与配置产生相同电脑落点。
- [ ] 暂停、失焦和最小化不推进电脑回合或动画。
- [ ] 棋局规则不直接修改金钱、心情或酒馆战绩。
- [ ] 自动化测试覆盖合法性、所有胜线、和局及电脑决策优先级。
- [ ] 集成测试覆盖酒馆进入、完整棋局、胜负结果和阶段结算。
