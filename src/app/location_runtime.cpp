    ...
    if (runtime.library_mode == LibraryWorkMode::selection) {
        ...
        const bool choose_reader =
            ...
            (IsMouseButtonPressed(MOUSE_BUTTON_LEFT) &&
             CheckCollisionPointRec(logical_mouse, scaled_rect(library::ui::reader_mode_button())));
        const bool choose_organizing =
            IsKeyPressed(KEY_TWO) ||
            (IsMouseButtonPressed(MOUSE_BUTTON_LEFT) &&
             CheckCollisionPointRec(logical_mouse, scaled_rect(library::ui::organizing_mode_button())));
        if (choose_reader) {
