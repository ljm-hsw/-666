...
unsigned int make_liars_dice_seed(unsigned int session_seed, int day, int result_id) noexcept {
    unsigned int seed = session_seed;
    seed ^= static_cast<unsigned int>(day) + 0x9e3779b9U + (seed << 6U) + (seed >> 2U);
    seed ^= static_cast<unsigned int>(result_id) + 0x85ebca6bU + (seed << 6U) +
            (seed >> 2U);
    return seed;
}
