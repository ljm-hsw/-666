...
普通启动进入当前完整循环：标题页点击“新游戏”进入第一日地图，选择一个白天地点完成工作；夜晚可以回家，也可以在酒馆完成一局五子棋或一场骗子骰子淘汰赛；第十日总结后进入占位主结局，不进入第十一日。
...
地点 UI 诊断截图可通过 `./build/pixel_town --capture-ui-diagnostics` 生成到被 Git 忽略的 `ui-diagnostics-captures/`；其中包含酒馆大厅、玩法选择、五子棋和骗子骰子页面。
