...
- Issues 23-29 已规划人物小人、轻量线性对话、有限室内移动和图书馆新增整理模式：餐馆与便利店对话拆为可并行的独立切片；主地图保持静态交互；图书馆和酒馆以方向键移动、静态家具碰撞和确定性 NPC 轨迹完成垂直切片，再由 Issue 16/17 人工批准手感、文案与最终视觉资源。
- Issue 23 已按 TDD 完成共享线性对话运行期和酒保端到端切片：对话输入模态拦截、UTF-8 三行布局、字形清单、角色缺图 fallback 和四张诊断截图均已验证。
- 2026-07-11 起暂缓视觉输入未就绪的 Issue 24-26；2026-07-13 团队提供的餐馆、家、图书馆、酒馆和便利店合成稿均已接入运行时，图书馆/酒馆静态碰撞基础完成后 Issue 27/28 恢复为 `ready-for-agent`，其中 28 仍依赖 27。分件生成、人工拼接、碰撞交接和恢复门槛见 [`docs/SCENE_ASSET_PRODUCTION_PLAN.md`](docs/SCENE_ASSET_PRODUCTION_PLAN.md)。
- 餐馆、便利店、图书馆和家现统一为“地图 → 场景大厅 → NPC 预留互动/返回 → 进入原玩法”的入口；进入大厅不消耗行动且不写入存档。酒馆保留专用大厅。
- 图书馆行动现提供并列的“读者咨询 / 书籍整理”模式：整理任务和书架由数据文件提供，错误归类可保留手中书重试，两种模式都通过核心统一行动结果结算；后续 Issue 27 只需把管理员互动接到该模式选择入口。
...
| [`docs/CHARACTER_DIALOGUE_PLAN.md`](docs/CHARACTER_DIALOGUE_PLAN.md) | 人物小人占位规格、共享对话、室内导航与 P4 实施切片 |
...
当前快捷键：`Enter` 确认/继续，`Esc` 返回或取消覆盖确认，`Space` 开始/完成地点模拟，`P` 暂停/继续，`M` 切换全局静音，`F3` 显示/隐藏已登记室内场景的碰撞调试层。鼠标仍是主要操作方式；窗口缩放使用整数倍显示，黑边区域点击不会触发控件。静音设置保存到应用目录旁的 `saves/settings.ini`。
...
地点 UI 诊断截图可通过 `./build/pixel_town --capture-ui-diagnostics` 生成到被 Git 忽略的 `ui-diagnostics-captures/`；其中包含四个通用场景大厅、便利店长结算摘要地图、五套碰撞覆盖图、酒馆大厅、玩法选择、五子棋、骗子骰子，以及酒保对话首句、中间句、末句和缺图 fallback 页面。
...
MVP 不包含小镇主地图或开放世界移动、通用寻路、复杂动态碰撞、复杂 NPC 关系、联网、多玩家、背包、成就、多语言、多存档槽、第三种酒馆游戏或运行时数值编辑器；图书馆和酒馆固定室内的方向键移动与静态碰撞属于有限范围。完整范围见 PRD。
