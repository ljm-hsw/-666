...
地点 UI 诊断截图可通过 `./build/pixel_town --capture-ui-diagnostics` 生成到被 Git 忽略的 `ui-diagnostics-captures/`；其中包含四个通用场景大厅、图书馆固定管理员及对话、便利店长结算摘要地图、五套可选美术诊断覆盖图、酒馆大厅、玩法选择、五子棋、骗子骰子、NPC 对话，以及餐馆/便利店雨天事件、图书馆旧集市事件和餐馆第十天回收对话。
