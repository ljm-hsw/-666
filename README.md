...
- Issues 23-29 已规划人物小人、轻量线性对话和图书馆新增整理模式：所有 NPC 固定在柜台或作者指定热点循环待机动画，玩家点击人物或场景热点进入对话和玩法；不加入主角移动、NPC 巡逻或运行时碰撞。
...
- Issue 26 已按 TDD 完成镇长/主角四句新游戏开场和两句回家独白：开场不修改第一日状态，回家独白关闭后只提交一次既有休息结果，第一日存档边界可无奖励重播。支线剧情继续延期。
- 2026-07-15 剧情规划已调整为“每个地点首访说明与指引，后续访问按游戏日进入日常/意外事件”。Issue 30-32 已拆分，先完成地点访问次数与存档迁移，再分别接入白天地点和酒馆/家的事件对话；首批事件不暗改玩法数值。
- 2026-07-13 团队提供的餐馆、家、图书馆、酒馆和便利店合成稿均已接入运行时。2026-07-14 Issue 27/28 按固定 NPC 待机与点击热点方案收口，不再依赖移动或碰撞。
- 餐馆、便利店、图书馆和家现统一为“地图 → 场景大厅 → NPC 互动/返回 → 进入原玩法”的入口；餐馆、便利店和图书馆已接正式线性对话，家仍为预留互动。进入大厅不消耗行动且不写入存档，酒馆保留专用大厅。
...
| [`docs/CHARACTER_DIALOGUE_PLAN.md`](docs/CHARACTER_DIALOGUE_PLAN.md) | 人物小人占位规格、共享对话、固定 NPC 待机/点击热点与 P4 实施切片 |
...
| [PRD](.scratch/pixel-town-ten-day-plan/PRD.md) | 产品范围、72 条用户故事和验收方向 |
| [实施 Issues](.scratch/pixel-town-ten-day-plan/issues/) | 28 个依赖有序的本地 Markdown issue |
...
当前快捷键：`Enter` 确认/继续，`Esc` 返回、取消、跳过对话或放弃当前玩法（不会关闭整个游戏），`Space` 触发当前固定 NPC 或开始/完成地点模拟，`P` 暂停/继续，`M` 切换全局静音，`F3` 显示/隐藏可选的室内美术诊断层。退出程序请使用系统窗口关闭按钮。鼠标仍是主要操作方式；窗口缩放使用整数倍显示，黑边区域点击不会触发控件。静音设置保存到应用目录旁的 `saves/settings.ini`。
...
地点 UI 诊断截图可通过 `./build/pixel_town --capture-ui-diagnostics` 生成到被 Git 忽略的 `ui-diagnostics-captures/`；当前共 45 张，其中包含四个通用场景大厅、餐馆厨师/便利店售货员/图书管理员/酒保的固定位置与第二待机帧、NPC/主角/镇长的固定首帧共享对话人物槽、便利店长结算摘要地图、五套可选美术诊断覆盖图、酒馆玩法，以及餐馆/便利店雨天事件、图书馆旧集市事件和餐馆第十天回收对话。
...
MVP 不包含主角室内或开放世界移动、NPC 巡逻、通用寻路、动态碰撞、复杂 NPC 关系、联网、多玩家、背包、成就、多语言、多存档槽、第三种酒馆游戏或运行时数值编辑器。完整范围见 PRD。
