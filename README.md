...
地点 UI 诊断截图可通过 `./build/pixel_town --capture-ui-diagnostics` 生成到被 Git 忽略的 `ui-diagnostics-captures/`；当前共 45 张，其中包含四个通用场景大厅、餐馆厨师/便利店售货员/图书管理员/酒保的固定位置与第二待机帧、共享对话人物槽、便利店长结算摘要地图、五套可选美术诊断覆盖图、酒馆玩法，以及餐馆/便利店雨天事件、图书馆旧集市事件和餐馆第十天回收对话。
